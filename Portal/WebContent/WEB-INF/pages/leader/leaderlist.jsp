...
<script>
	...
	<div class="bg_w1284">
		<div class="wrap">
			...
			<div class="content">
				<div class="clearfix">
					<c:if test="${leaderName ne '陈孝来' }">
						<!--mainList Start-->
						<div class="mainList" id="lH">
					</c:if>
					<c:if test="${leaderName eq '陈孝来' }">
						<!--mainList Start-->
						<div class="mainList" id="lH" style="display: none;">
					</c:if>
					<div class="breadCutNav">
						...
						<span> &gt;</span>
						<a href="${pageContext.request.contextPath}/openCatalog/index.html">政务公开</a>
						<span> &gt;</span>
						<a href="javaScript:;" class="cur">主任网页</a>
					</div>
					<!--personalIntro Start-->
					<div class="personalIntro clearfix">
						<div class="photo">
							<img alt="领导" src="${leaderPhotosUrl}" width="145px" height="208px" />
							<h3>${leaderName}</h3>
							<p>${leaderPost}</p>
						</div>
						<div class="intro" style="width:555px;word-wrap:break-word;word-break:break-all;">
							<h2 class="clearfix">
								<span>
									<b class="icon i_triangleBlue"></b>
									简历
								</span>
							</h2>
							<div>${leaderResume.replace("&nbsp;","&emsp;")}</div>
						</div>
					</div>
					<!--personalIntro End-->
					<div class="activityList list">
						...
						<ul>
							<c:forEach items="${NewsList}" var="News" varStatus="newsID">
								<li>
									<a target="_blank" href="${pageContext.request.contextPath}/articles/${leaderId}/${News.newsID}.html"> &bull;&nbsp;${News.newsTitle}</a>
								</li>
							</c:forEach>
						</ul>
						<ut:PortalPager url="${pageContext.request.contextPath}/leaders/${leaderId }" pagesize="${pagesize}" totalLines="${count}" curpage="${curpage}" />
					</div>
				</div>
				...
				<div class="subList personalList" id="rH">
					...
					<div class="img-scroll_v clearfix" id="img-scroll_v">
						<a href="javascript:;" class="up scrollBtn" id="up"></a>
						<a href="javascript:;" class="down scrollBtn" id="down"></a>
						<div class="img-list_v" id="scrollUp">
							<ul class="clearfix" id="scrollUp01">
								<c:forEach items="${leaderList}" var="Leader" varStatus="leaderId">
									<li>
										<a href="${pageContext.request.contextPath}/leaders/${Leader.leaderId}_.html">
											<img alt="领导" src="${prourl}${Leader.leaderPhotosUrl}" />
										</a>
										<div>
											<h4>
												<a href="${pageContext.request.contextPath}/leaders/${Leader.leaderId}_.html"> ${Leader.leaderName} </a>
											</h4>
