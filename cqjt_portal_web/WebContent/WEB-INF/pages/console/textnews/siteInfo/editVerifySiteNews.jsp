	...
	<div>
		...
		<div class="path">
			<b class="icon i-home"></b>
			 ...
			 信息报送
			<span>-></span>
			<a href="siteTextNews.xhtml?face=getVerifySiteInfoView&flag=2">网站信息校编 </a>
			<span>-></span>
			编辑稿件
		</div>
		<!--path End-->
		...
		<div class="column">
			<div class="title">
				<h2>
					<b class="icon i-search"></b>网站信息校编编辑
				</h2>
			</div>
			<div class="con docEdit">
				<form id="textNewsForm" enctype="multipart/form-data" method="POST">
					<input type="hidden" name="newsContent" id="newsContent" /> <input
						type="hidden" name="newsTagsStr" id="newsTags" /> <input
						type="hidden" name="flag" id="newsFlag" /> <input type="hidden"
						name="entryDept" value="${userDeptID}" /> <input type="hidden"
						name="entryDate" value="${textNewsEditInfo.entryDate}" /> <input
						type="hidden" name="newsId" value="${textNewsEditInfo.newsId}" />
					<input type="hidden" name="outerNewsId"
						value="${textNewsEditInfo.outerNewsId}" /> <input type="hidden"
						name="innerCategory" class="input input-small" /> <input
						type="hidden" name="outerCategory" class="input input-small"
						value="${textNewsEditInfo.outerCategory}" />
						<input type="hidden" name="govUseFlag" value="${textNewsEditInfo.govUseFlag}">
					<ul>
						<li><label><span style="color: red">*</span>标题：</label> <input
							type="text" name="newsTitle" class="input input-large"
							value="${textNewsEditInfo.newsTitle}"></li>
						<li><label>正文标题：</label> <input
						type="text" name="subNewsTitle" class="input input-large"
						value="${textNewsEditInfo.subNewsTitle}"></li>
						<li><label><span style="color: red">*</span>作者：</label> <input
							type="text" name="newsAuthor"
							value="${textNewsEditInfo.newsAuthor}" class="input input-small">
							<label><span style="color: red"></span>审核人：</label> <select
							class="select-small" id="approvaler">
						...
						<li><label>来源出处：</label> <input type="text" name="newsSource"
							value="${textNewsEditInfo.newsSource}" class="input input-small">

							<label><span style="color: red">*</span>投稿部门：</label> <select
							class="select-small" disabled="disabled" name="entryDept">
								<option value="${textNewsEditInfo.entryDept}">${textNewsEditInfo.deptName}</option>
						</select> <input type="hidden" name="deptName"
							value="${textNewsEditInfo.deptName}" class="input input-small">
							<input type="hidden" name="entryUser"
							value="${textNewsEditInfo.entryUser}" class="input input-small">
						</li>
						<li><label>投稿时间：</label> <input type="text" name="createDate"
							value="${textNewsEditInfo.createDate}" class="input input-small">
						</li>
						<li class="clearfix"><label class="l"><span
						...
						<li><input type="hidden" name="newsTagsStr"
							value="${textNewsEditInfo.newsTagsStr}" /> <label><span
								style="color: red">*</span>信息栏目：</label> <select class="select-small"
							name="category" id="newsCategory" onchange="choose()">
								<option value="0">-=请选择栏目=-</option>
								<c:forEach items="${textCategorys}" var="c">
									<option value="${c.categoryId}">${c.categoryName}</option>
								</c:forEach>
						</select> <input type="hidden" id="categoryEdit"
							value="${textNewsEditInfo.category}"></li>
						<li id="showPhotoState"><label>主题图片：</label>
						...
						<li id="showPublicState"><label><span
								style="color: red">*</span>是否上外网：</label> <input type="hidden"
							id="publicChecked" value="${textNewsEditInfo.isPublic}">
							<input type="radio" name="isPublic" id="public" value="1"
							onclick="selectOuter(0);"><span>是</span>&nbsp;&nbsp; <input
							type="radio" name="isPublic" id="unPublic" value="0"
							onclick="selectOuter(1);"><span>否</span>
							 <!-- 是否置顶 -->
                 <li id="isStickState">
                    ...
                    <input type="radio" name="stickState" id="stickState" value="0" ><span>否</span>
							...
							<div id="outer" style="display: none">
								<label>外网栏目：</label> <select class="select-small"
									name="categoryOuter" id="newsCategoryOuter">
						<c:choose>
						...
						<li id="#"><label style="width: 120px"><span
								style="color: red">*</span>是否送办公室校编：</label> <input type="hidden"
							id="isCheckAgainChecked" value="${textNewsEditInfo.isCheckAgain}">
							<input type="radio" name="isCheckAgain" id="isCheckAgain"
							value="1" onclick="selectVerify(1);"><span>是</span>&nbsp;&nbsp;
							<input type="radio" name="isCheckAgain" id="isCheckAgain"
							value="0" onclick="selectVerify(0);"><span>否</span>
							<div id="verifyShowStatus">
								<label>校编人：</label> <select class="select-small"
									name="compiler">
									<c:if test='${empty textNewsEditInfo.compiler}'>
										<option value="">-=请选择校编人=-</option>
										<c:forEach items="${approvers}" varStatus="status">
											<option value="${status.current.userName}">${status.current.userName}</option>
										</c:forEach>
									</c:if>
						...
						<li><label></label> <a href="javascript:;" class="btn-gray" id="isVerifyShow" 
							onclick="updateSiteNews(19)"><b class="icon i-send"></b>送校编</a>
							<a href="javascript:;" class="btn-gray" onclick="updateSiteNews(3)"><b
								class="icon i-send"></b>校编</a> <a href="javascript:;"
							class="btn-gray" onclick="updateSiteNews(7)"><b
								class="icon i-del"></b> 退稿</a> <a href="javascript:history.go(-1);"
							class="btn-gray"><b class="icon i-back"></b>返 回</a></li>
						<li><label>退稿原因：</label>
						</li>
						<li><textarea
								style="width: 30%; resize: none; margin-left: 70px;"
								name="rejectReason"></textarea></li>
					</ul>
				</form>
			</div>
		</div>
		...
		<div class="column" style="width: 65%">
			<div class="title">
				<h2>
					<b class="icon i-search"></b>信息报送处理记录表
				</h2>
			</div>
			<div class="con">
					...
					<tbody id="tbodyVideo">
						<c:forEach items="${newsRecord}" var="tn" varStatus="status">
							<tr>
								...
								<td>${tn.updateDate}</td>
								<td>
								<c:if test='${tn.flag=="19"}'>送校编</c:if>
								<c:if test='${tn.flag=="18"}'>归档</c:if> <c:if
										test='${tn.flag=="17"}'>发布</c:if> <c:if
										test='${tn.flag=="16"}'>复原</c:if> <c:if
										test='${tn.flag=="15"}'>已归档删除</c:if> <c:if
										test='${tn.flag=="14"}'>校编时退稿</c:if> <c:if
										test='${tn.flag=="13"}'>分拣时退稿</c:if> <c:if
										test='${tn.flag=="12"}'>校编时删除</c:if> <c:if
										test='${tn.flag=="11"}'>分拣时删除</c:if> <c:if
										test='${tn.flag=="10"}'>上报市府</c:if> <c:if
										test='${tn.flag=="9"}'>上报市委</c:if> <c:if
										test='${tn.flag=="8"}'>上报</c:if> <c:if test='${tn.flag=="7"}'>待校编</c:if>
									<c:if test='${tn.flag=="6"}'>采编时删除</c:if> <c:if
										test='${tn.flag=="5"}'>采编</c:if> <c:if test='${tn.flag=="4"}'>修改</c:if>
									<c:if test='${tn.flag=="3"}'>校编</c:if> <c:if
										test='${tn.flag=="2"}'>分拣</c:if> <c:if test='${tn.flag=="1"}'>投稿</c:if>
									<c:if test='${tn.flag=="0"}'>存草稿</c:if></td>
							</tr>
						</c:forEach>
					</tbody>
				</table>
...
window.APP_HOME_URL = "${pageContext.request.contextPath}";
	function show1() {
		window
				.open(
						'${StaticResourcePath}/${textNewsEditInfo.photoUrl}',
						'add',
	...
	$(function() {
		...
		//判断 是否再次送校编
		var isCheckAgain = $("#isCheckAgainChecked").attr("value");
		$("input[name='isCheckAgain']:radio").each(function() {
			if ($(this).val() == isCheckAgain) {
				$(this).attr("checked", "checked");
			}
		});
		//当isCheckAgain为1时 显示校编人
		if (isCheckAgain == "1") {
			$('#verifyShowStatus').show();
			$('#isVerifyShow').show();
		}else{
			$('#verifyShowStatus').hide();
			$('#isVerifyShow').hide();
		}
	...
	function updateSiteNews(flag) {
		...
		$("input[name='outerCategory']").val(
				$('#newsCategoryOuter option:selected').text());
		var outerCategory = $("input[name='outerCategory']").val();
		//校验输入框
		if (newstitle == "" && newstitle.length == 0) {
			$("input[name='newsTitle']").next(".error").remove();
			$("input[name='newsTitle']").after(
					"<span class='error'>标题不能为空</span>");
			return;
		}
		if (newsauthor == "" && newsauthor.length == 0) {
			$("input[name='newsAuthor']").next(".error").remove();
			$("input[name='newsAuthor']").after(
					"<span class='error'>作者不能为空</span>");
			return;
		}
		/*
		if(approvaler ==""){
		...
		$("#newsFlag").val(flag);
		 ...
		 $("input[name='isCheckAgain']:radio").each(function() {
				if (this.checked) {
					if ($(this).val() == '1' && flag == '3') {
					asyncbox.confirm("请确认不再送校编?","确认窗口",function(action) {
					if (action == "ok") {
						document.location.href = "siteTextNews.xhtml?face=getVerifySiteInfoView";
						}else{
							alert(action=="ok");
							return;
						}
...
<script type="text/javascript"
	src="${pageContext.request.contextPath}/plugin/ueditor/ueditor.config.js"></script>
<script type="text/javascript"
	src="${pageContext.request.contextPath}/plugin/ueditor/ueditor.all.min.js"></script>
</html>
